...
|--------|----------|--------|
| `SSH_HOST` | IP-адрес или домен сервера | `51.250.123.45` или `aqstream.ru` |
| `SSH_USER` | Имя пользователя SSH | `aqstream` |
| `SSH_KEY` | Приватный SSH ключ | Содержимое `~/.ssh/id_ed25519` |

**Получить приватный ключ:**

```bash
# На локальной машине (где настроен доступ к серверу)
cat ~/.ssh/id_ed25519
```

Скопировать **весь** вывод, включая строки `-----BEGIN OPENSSH PRIVATE KEY-----` и `-----END OPENSSH PRIVATE KEY-----`.

**Важно:** Используйте ключ, который уже добавлен в `~/.ssh/authorized_keys` на сервере.
