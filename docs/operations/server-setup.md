...
```bash
# На локальной машине — показать приватный ключ
cat ~/.ssh/id_ed25519
```

Пример вывода (скопировать **полностью**):
...
**Важно:**
- Копировать **весь** вывод, включая строки `-----BEGIN` и `-----END`
- Не добавлять лишние пробелы или переводы строк
- Использовать ключ, который уже добавлен в `~/.ssh/authorized_keys` на сервере

#### Проверка что ключ работает

```bash
# Проверить подключение с локальной машины
ssh -i ~/.ssh/id_ed25519 aqstream@51.250.123.45 "echo 'Connection OK'"
```

Если выводит `Connection OK` — ключ настроен правильно.
