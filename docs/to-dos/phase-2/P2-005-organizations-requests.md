...
- [ ] Пользователь может подать запрос на создание организации (`POST /api/v1/organization-requests`)
- [ ] Запрос содержит: название, slug (URL), описание
- [ ] Slug проверяется на уникальность (среди организаций и pending запросов)
- [ ] Slug валидируется: только lowercase, цифры, дефис; длина 3-50 символов
- [ ] Пользователь может иметь только один pending запрос
- [ ] Пользователь видит статус своего запроса

### Рассмотрение админом

- [ ] Админ видит список всех pending запросов (`GET /api/v1/organization-requests`)
- [ ] Админ может одобрить запрос (`POST /api/v1/organization-requests/{id}/approve`)
- [ ] Админ может отклонить запрос с указанием причины (`POST /api/v1/organization-requests/{id}/reject`)
- [ ] При одобрении пользователь получает уведомление в Telegram
- [ ] При отклонении пользователь получает уведомление с причиной

### После одобрения

- [ ] Одобренный пользователь может создать организацию
- [ ] Slug из запроса резервируется до создания организации (7 дней)
